...
export default function Sidebar() {
  ...
  const bg = isDark ? "bg-[#111111] border-white/[0.05]" : "bg-white border-gray-200";
  const txt = isDark ? "text-white/60 hover:text-white hover:bg-white/[0.03]" : "text-gray-500 hover:text-gray-900 hover:bg-gray-100";
  const activeCls = "bg-[#FA3633]/10 text-[#FA3633]";
  const borderCls = isDark ? "border-white/[0.03]" : "border-gray-200";
    ...
    <aside className={`${collapsed ? "w-[70px]" : "w-[240px]"} ${bg} border-r flex flex-col transition-all duration-300`}>
      ...
      <div className={`py-4 px-3 border-t ${borderCls} space-y-1`}>
        ...
        <button onClick={handleLogout}
          className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium ${isDark ? "text-white/60 hover:text-red-400 hover:bg-white/[0.03]" : "text-gray-500 hover:text-red-500 hover:bg-gray-100"} transition-colors w-full`}>
