...
export default function DashboardClient({ data }: { data: DashboardData }) {
  ...
  const card = isDark ? "bg-[#1a1a1a] border-white/[0.025]" : "bg-white border-gray-200 shadow-sm";
  const txt = isDark ? "text-white" : "text-gray-900";
  const txtSub = isDark ? "text-white/50" : "text-gray-500";
  const txtMuted = isDark ? "text-white/30" : "text-gray-400";
  const tableBorder = isDark ? "border-white/[0.025]" : "border-gray-200";
  const tableRowHover = isDark ? "hover:bg-white/[0.02]" : "hover:bg-gray-50";

  const stats = [
    { label: "\u0e22\u0e2d\u0e14\u0e23\u0e27\u0e21\u0e40\u0e14\u0e37\u0e2d\u0e19\u0e19\u0e35\u0e49", value: `\u0e3f${(data.totalAmount || 0).toLocaleString()}`, change: "\u2197 0%", icon: "\ud83d\udcc8", color: "text-[#FA3633]" },
    { label: "\u0e08\u0e33\u0e19\u0e27\u0e19\u0e43\u0e1a\u0e40\u0e2a\u0e23\u0e47\u0e08", value: `${data.receiptCount || 0} \u0e43\u0e1a`, change: "\u2197 0%", icon: "\ud83d\udcc4", color: "text-blue-400" },
    { label: "\u0e40\u0e09\u0e25\u0e35\u0e48\u0e22\u0e15\u0e48\u0e2d\u0e43\u0e1a", value: `\u0e3f${(data.avgPerReceipt || 0).toLocaleString()}`, icon: "\ud83d\udccb", color: "text-green-400" },
    { label: "\u0e2b\u0e21\u0e27\u0e14\u0e2b\u0e21\u0e39\u0e48", value: `${(data.categoryCount || 0)} \u0e2b\u0e21\u0e27\u0e14`, icon: "\ud83d\udcc1", color: "text-purple-400" },
  ];
        ...
        {stats.map((s, i) => (
          <div key={i} className={`${card} border rounded-xl p-5`}>
            <div className="flex items-center justify-between mb-3">
              <span className={`text-2xl p-2 rounded-lg ${isDark ? "bg-white/[0.02]" : "bg-gray-100"}`}>{s.icon}</span>
            ...
            <div className="space-y-3">
              {(data.categoryData || []).slice(0, 5).map((c, i) => (
                <div key={i} className="flex items-center gap-3">
                  ...
                  <span className={`text-sm font-medium ${txt}`}>{"\u0e3f"}{(c.amount || 0).toLocaleString()}</span>
                </div>
              ))}
            </div>
